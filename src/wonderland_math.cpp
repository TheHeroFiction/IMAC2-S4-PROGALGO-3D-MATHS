...
float WonderlandEngine::get_normal(float mu, float sigma) {
    ...
    float z0 = std::sqrtf(-2.0f * std::logf(u1)) * std::cosf(2.0f * static_cast<float>(M_PI) * u2);
...
float WonderlandEngine::get_exponential(float lambda) {
    float u = get_uniform();
    // Inverse transform
    return -std::logf(1.0f - u) / lambda;
}

float WonderlandEngine::get_cauchy(float x0, float gamma) {
    float u = get_uniform();
    // Inverse transform
    return x0 + gamma * std::tanf(static_cast<float>(M_PI) * (u - 0.5f));
}
...
float WonderlandEngine::get_beta(float alpha, float beta_param) {
    ...
    float c_max = std::powf((alpha - 1.0f) / (alpha + beta_param - 2.0f), alpha - 1.0f) *
                  std::powf((beta_param - 1.0f) / (alpha + beta_param - 2.0f), beta_param - 1.0f);

    while (true) {
        float x = get_uniform();
        float y = get_uniform() * c_max; 
        float f_x = std::powf(x, alpha - 1.0f) * std::powf(1.0f - x, beta_param - 1.0f);
...
int WonderlandEngine::get_geometric(float p) {
    ...
    float u = get_uniform();
    // Inverse transform
    return static_cast<int>(std::floorf(std::logf(u) / std::logf(1.0f - p)));
}
