...
public:
    ...
    // 2. Exponential Distribution (Inverse transform sampling)
    float get_exponential(float lambda);

    // 3. Cauchy Distribution (Inverse transform sampling)
    float get_cauchy(float x0, float gamma);

    // 4. Beta Distribution (Acceptance-Rejection method)
    // Only implemented for alpha > 1 and beta > 1 for simplicity in this context
    float get_beta(float alpha, float beta_param);

    // --- DISCRETE DISTRIBUTIONS ---

    // 5. Poisson Distribution (Knuth's algorithm)
    int get_poisson(float lambda);

    // 6. Geometric Distribution (Inverse transform sampling)
    // Returns the number of failures before the first success
    int get_geometric(float p);

    // 7. Binomial Distribution (Sum of n Bernoulli trials)
    int get_binomial(int n, float p);

    // 8. Random Permutation (Fisher-Yates shuffle)
